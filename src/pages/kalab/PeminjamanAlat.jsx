...
export const loader = async ({request}) => {
    ...
    try {
        const result = await customFetch.get(`v1/peminjaman/alat/validasi-laboran`, {
    ...
    return (
        <Wrapper>
            <div className="mx-5 my-10 bg-white shadow-lg py-5 px-5 rounded-sm max-w-[65.625em] flex flex-col h-[30em]">
                ...
                <div className="overflow-auto max-w-full flex flex-col justify-between h-[25em]">
                    <table className=" table-auto w-full min-w-full border border-collapse my-5 text-center text-sm">
                        ...
                        {data.map((val) => {
                            return (
                                <tr key={val.id} className={"border"}>
                                    <td className="border px-2 truncate">{no++}</td>
                                    <td className="border px-2 truncate">{val.nama}</td>
                                    <td className="border px-2 truncate">{val.nidn}</td>
                                    <td className="border px-2 truncate">{val.keperluan}</td>
                                    <td className="border px-2 truncate">{val.jenis_barang}</td>
                                    ...
                                    <td className="border px-2 truncate"> {val.status == "validasi_laboran" && "validasi laboran"} </td>
                                    <td className="border px-2 truncate"> {`${val.jam_mulai || "00:00"} - ${val.jam_selesai || "00:00"}`} </td>
                                    <td className=" px-2">
                                        {val.filename && (
                                            <a
                                    ...
                                    <td className="p-2 border text-white flex items-center text-center justify-center">
