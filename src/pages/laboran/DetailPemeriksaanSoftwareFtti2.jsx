...
const DetailPemeriksaanSoftwareFtti2 = () => {
            ...
            <table className="table-auto border border-collapse">
              <thead className="border border-collapse bg-gray-200">
                <tr>
                  ...
                  <th className="border py-2 px-1 text-sm">Balsamic</th>
                  <th className="border py-2 px-1 text-sm">Pycharm</th>
                  <th className="border py-2 px-1 text-sm">Cpu Simulator</th>
                  <th className="border py-2 px-1 text-sm">Photoshop</th>
                  <th className="border py-2 px-1 text-sm">Python</th>
                  <th className="border py-2 px-1 text-sm">veyon</th>
                  <th className="border py-2 px-1 text-sm">Composer</th>
                  <th className="border py-2 px-1 text-sm">Arduino IDE</th>
                ...
                {detailPemeriksaan.map((val) => {
                  return (
                    <tr key={val.id}>
                      ...
                      <td className="border px-2 text-xs">{val.balsamic}</td>
                      <td className="border px-2 text-xs">{val.pycharm}</td>
                      <td className="border px-2 text-xs">
                        {val.cpu_simulator}
                      </td>
                      <td className="border px-2 text-xs">{val.photoshop}</td>
                      <td className="border px-2 text-xs">{val.python}</td>
                      <td className="border px-2 text-xs">{val.veyon}</td>
                      <td className="border px-2 text-xs">{val.composer}</td>
                      <td className="border px-2 text-xs">{val.arduino_ide}</td>
                      <td className="border py-2 text-lg">
                        <Link
                          to={`/admin/dashboard-laboran/${id}/pemeriksaan/software/${user[0].id}/detail-ftti2/${val.id}/edit`}
                        >
                          <button
                            type="button"
...
export default DetailPemeriksaanSoftwareFtti2